...
python -m uv run mod-manual scaffold category <modid> <category_id> --name "Getting Started" --locale en_us
python -m uv run mod-manual scaffold entry <modid> <entry_id> --category <category_id> --name "First Steps" --locale en_us
```

## 原稿フォーマット

- `book.yml`: 書籍メタデータ、`book_id`、`book_namespace`、`locales`
- `locales/<locale>/categories/*.yml`: locale ごとのカテゴリ定義
- `locales/<locale>/entries/**/*.yml`: locale ごとのエントリ定義
- `locales/<locale>/pages/<entry_id>/*.md`: YAML front matter + 本文
